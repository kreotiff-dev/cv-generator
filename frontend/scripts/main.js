$(document).ready(function() {
    ...
    // Получаем userId из URL (предполагается, что userId включен в URL, например, как параметр)
    const params = new URLSearchParams(window.location.search);
    const userId = params.get("userId");

    // Функция загрузки данных профиля
    function loadProfileData(language) {
        const filePath = language === 'ru' ? `/public/resumes/${userId}/ru.json` : `/public/resumes/${userId}/en.json`;
